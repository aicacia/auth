...
func CreateEmail(applicationId, userId int32, email string) (EmailRowST, error) {
	confirmationToken, err := util.GenerateRandomHex(8)
	if err != nil {
		var empty EmailRowST
		return empty, err
	}
...
func SetEmailConfirmation(userId, id int32, token string) (EmailRowST, error) {
...
func ConfirmEmail(userId, id int32, token string) (EmailRowST, error) {
	return Get[EmailRowST](`UPDATE emails SET
		confirmed=true
		WHERE user_id=$1 
			AND id=$2
