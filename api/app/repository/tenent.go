...
func UpdateTenent(id int32, update UpdateTenentST) (*TenentRowST, error) {
	return GetOptional[TenentRowST](`UPDATE tenents SET
		...
		reset_expires_in_seconds = COALESCE($14, reset_expires_in_seconds)
		WHERE id = $1
		RETURNING *;`,
		id, update.Description, update.Uri, update.AuthorizationWebsite, update.RegistrationWebsite, update.EmailEndpoint, update.PhoneNumberEndpoint, update.ClientId, update.Algorithm, update.PublicKey, update.PrivateKey, update.ExpiresInSeconds, update.RefreshExpiresInSeconds,
	)
}
